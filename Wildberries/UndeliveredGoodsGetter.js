const openAllItems = () => {
    ...
    document.querySelector(".Dropdown-list__m2s501tI-S")?.lastElementChild?.lastChild?.click()
}
...
const getItems = () => {
	...
	const place = document.querySelectorAll(".Application-info-item__value-black__3sCu7Lg3uC")[7].textContent
	const req = document.querySelector(".Text--black__2QzLgsh8wU").textContent.split("№")[1]
	let unfinishedItems = ""
	items.forEach(item => {
...
const addButton = () => {
    ...
    button.appendChild(document.createTextNode("Копировать недоставленные товары"))
    ...
    document.querySelector(".Details-application-page__upload-btn")?.appendChild(button)
}
